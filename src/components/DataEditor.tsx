...
const useHistory = (initialState: EditorState, onSave: (state: EditorState) => void) => {
    ...
    // Note: We deliberately DO NOT reset history when initialState changes.
    // This allows the component to persist its local history even if the parent (App) 
    // updates its data (e.g. during an auto-save loop).
    // The component is fully reset only when the Parent changes the `key` prop.
...
export const DataEditor: React.FC<DataEditorProps> = ({ initialData, initialHeaders, onReset, fileName, onGoHome, bloInfo, onFileLoad, isLoading, error, onSave }) => {
    ...
    const performSave = useCallback((d: VoterRecord[], h: string[]) => {
        setIsSaving(true);
        onSave(d, h);
        
        // Simulate a small delay for visual feedback, though sync save is instant
        setTimeout(() => {
    ...
    const handleHistorySave = useCallback((state: EditorState) => {
        ...
        saveTimeoutRef.current = setTimeout(() => {
            saveTimeoutRef.current = null; // Mark as done to prevent double save on unmount/cleanup
            performSave(state.data, state.headers);
        }, 2000); 
    }, [performSave]);
    
    const { state, setState, undo, redo, canUndo, canRedo } = useHistory({ data: initialData, headers: initialHeaders }, handleHistorySave);
    const { data, headers } = state;
    const ai = useGemini();
    
    // Ensure we save when unmounting or if onSave changes while a save is pending
    const latestStateRef = useRef(state);
    useEffect(() => { latestStateRef.current = state; }, [state]);

    useEffect(() => {
        return () => {
            // Only trigger immediate save if there is a pending timeout
            if (saveTimeoutRef.current) {
                clearTimeout(saveTimeoutRef.current);
                saveTimeoutRef.current = null;
                onSave(latestStateRef.current.data, latestStateRef.current.headers);
            }
        };
    }, [onSave]);
            ...
            <header className="flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-4">
                     ...
                     <div className="flex items-center gap-2 text-sm text-slate-400 bg-slate-800/50 px-3 py-1.5 rounded-full border border-slate-700">
                         {isSaving ? (
                             <>
                                <SpinnerIcon className="w-4 h-4 animate-spin text-yellow-400" />
                                <span className="text-yellow-400">Saving...</span>
                             </>
                         ) : (
                             <>
                                <CheckCircleIcon className="w-4 h-4 text-green-400" />
                                <span className="text-green-300 font-medium">{lastSaved ? `Saved ${lastSaved.toLocaleTimeString()}` : 'Ready'}</span>
                             </>
                         )}
                     </div>
